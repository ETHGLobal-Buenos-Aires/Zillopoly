...
function encodeNumber(value2, { signed, size: size6 = 256 }) {
...
function decodeParameter(cursor3, param, { staticPosition }) {
  const arrayComponents = getArrayComponents(param.type);
  if (arrayComponents) {
    const [length, type] = arrayComponents;
...
function decodeBool(cursor3) {
  return [bytesToBool(cursor3.readBytes(32), { size: 32 }), 32];
}
function decodeBytes(cursor3, param, { staticPosition }) {
  ...
  const value2 = bytesToHex(cursor3.readBytes(Number.parseInt(size7, 10), 32));
  return [value2, 32];
}
function decodeNumber(cursor3, param) {
  const signed = param.type.startsWith("int");
  const size7 = Number.parseInt(param.type.split("int")[1] || "256", 10);
  const value2 = cursor3.readBytes(32);
  return [
...
function decodeTuple(cursor3, param, { staticPosition }) {
...
function decodeString(cursor3, { staticPosition }) {
  const offset = bytesToNumber(cursor3.readBytes(32));
  const start = staticPosition + offset;
  cursor3.setPosition(start);
  const length = bytesToNumber(cursor3.readBytes(32));
...
function checkScalarValue(value, scalar3) {
  switch (scalar3) {
    case ScalarType.DOUBLE:
...
function scalarTypeDescription(scalar3) {
  switch (scalar3) {
    case ScalarType.STRING:
      return "string";
    case ScalarType.BOOL:
...
function writeMessageField(writer, opts, field, message4) {
...
function assertExtendee(extension, message6) {
...
function decodeEventLog(parameters) {
  const { abi: abi7, data: data3, strict: strict_, topics } = parameters;
  const strict = strict_ ?? true;
  const [signature, ...argTopics] = topics;
  if (!signature)
    throw new AbiEventSignatureEmptyTopicsError({ docsPath: docsPath2 });
  const abiItem = abi7.find((x) => x.type === "event" && signature === toEventSelector(formatAbiItem2(x)));
...
var onBatchGamesCreated = async (runtime4, payload) => {
  runtime4.log("=== Batch Games Created Event Detected ===");
  try {
    ...
    const eventData = decodedLog.args;
    runtime4.log(`Player: ${eventData.player}`);
    runtime4.log(`Start Game ID: ${eventData.startGameId.toString()}`);
    runtime4.log(`End Game ID: ${eventData.endGameId.toString()}`);
    runtime4.log(`Timestamp: ${eventData.timestamp.toString()}`);
    runtime4.log(`Block Number: ${payload.blockNumber}`);
    runtime4.log(`Transaction Hash: ${payload.transactionHash}`);
    const numGames = Number(eventData.endGameId - eventData.startGameId + 1n);
    runtime4.log(`Fetching ${numGames} listings from API...`);
    const http = new cre.capabilities.HTTPCapability;
    const evm = new cre.capabilities.EVMCapability;
    const results = [];
    for (let i = 0;i < numGames; i++) {
      const currentGameId = eventData.startGameId + BigInt(i);
      runtime4.log(`Fetching listing ${i + 1}/${numGames} for game ID ${currentGameId}...`);
      const response = await http.fetch({
        url: `${runtime4.config.apiUrl}/api/random-listing`,
        method: "GET",
        headers: {}
      });
      if (!response.ok || response.statusCode !== 200) {
        runtime4.log(`Failed to fetch listing ${i + 1}: ${response.statusCode}`);
        results.push({ gameId: currentGameId.toString(), status: "failed", error: "API request failed" });
        continue;
      }
      const listingData = JSON.parse(response.body);
      if (!listingData.success) {
        runtime4.log(`Failed to get listing ${i + 1}: ${listingData}`);
        results.push({ gameId: currentGameId.toString(), status: "failed", error: "No listing data" });
        continue;
      }
      runtime4.log(`Got listing from ${listingData.city}: \$${listingData.contractData.displayedPrice}`);
      const calldata = encodeFunctionData({
        abi: ZILLOPOLY_ABI,
        functionName: "initializeGame",
        args: [
          currentGameId,
          listingData.contractData.listingId,
          BigInt(listingData.contractData.displayedPrice)
        ]
      });
      runtime4.log(`Initializing game ${currentGameId} with listing ${listingData.listing.zpid}...`);
      const txResponse = await evm.write({
        chainName: runtime4.config.chainName,
        to: runtime4.config.zillopolyAddress,
        data: calldata
      });
      runtime4.log(`Game ${currentGameId} initialized. Tx: ${txResponse.transactionHash}`);
      results.push({
    ...
    runtime4.log(`Error processing batch games: ${error4}`);
    return JSON.stringify({ error: String(error4) });
  }
};
var initWorkflow = (config2) => {
  const evm = new cre.capabilities.EVMCapability;
  return [
    cre.handler(evm.logTrigger({
      chainName: config2.chainName,
      addresses: [config2.zillopolyAddress],
      topics: {
